...
class ShapeNetViewpointsBase(Dataset):
    ...
    def __init__(self, root, split="training", transform=None, filter_name=None, num_non_manifold_points=2048, dataset_size=None, in_normal_field=None, **kwargs):
        ...
        if split in ["train", "training"]:
            ...
            for path in glob.glob(os.path.join(self.root,f"*/convonet/{version}/val.lst")):
                self.filelists.append(path)
        elif split in ["validation", "val", "test", "testing", "test100"]: # no validation set
            # for path in glob.glob(os.path.join(self.root,f"*/convonet/{version}/test.lst")):
            for path in glob.glob(os.path.join(self.root,f"*/test100.lst")):
                self.filelists.append(path)
        self.filelists.sort()
    ...
    def get(self, idx):
        ...
        filename = self.filenames[idx]
        manifold_data =np.load(os.path.join(filename, "scan", "4.npz"))
        points_shape = manifold_data["points"]
        points_shape = np.matmul(points_shape,R)
        ...
        normals_shape = np.matmul(normals_shape,R)
...
class ShapeNetViewpoints(ShapeNetViewpointsBase):
